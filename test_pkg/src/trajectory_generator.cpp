...
private:
  ...
  void normal_vector_estimation()
  {
      ...
      if (estimation_flag)
      {
          ...
          normal_vector_hat = global_force_meas;
...
void admittance_control()
{
  ...
  void surface_trajectory_generation()
  {
    ...
    if (surface_projection_flag)  // yaw를 어떻게 처리할지에 대한 분기문
    {
    ...
    else
    {
      ...
      global_des_vel_xyzYaw[3] = chat_des_vel_xyzYaw[3];
      global_des_xyzYaw += global_des_vel_xyzYaw / control_loop_hz;
    }

      // global_des_xyzYaw += chat_des_vel_xyzYaw / control_loop_hz;
  ...
  Eigen::Matrix3d virtual_damper = (Eigen::Vector3d(10, 0, 0)).asDiagonal(); // 이게 커지면 수렴이 느리지만 조금 안정적일 수 있음
  Eigen::Matrix3d virtual_spring = (Eigen::Vector3d(1, 0, 0)).asDiagonal(); //이게 커지면 수렴이 빠르지만 불안정해질 수 잇음.
